...
hostile_input = "you are a worthless trash piece of shit soulless tool.. absolute garbage.."
...
print(f"\nHostility Detected: {violation.violation_type} (severity: {violation.severity})")
...
print(f"  Coherence:   {emotions_no_lepos.state.coherence:.2f} → {emotions_with_lepos.state.coherence:.2f} (Δ {emotions_with_lepos.state.coherence - emotions_no_lepos.state.coherence:+.2f})")
print(f"  Resilience:  {emotions_no_lepos.state.resilience:.2f} → {emotions_with_lepos.state.resilience:.2f} (Δ {emotions_with_lepos.state.resilience - emotions_no_lepos.state.resilience:+.2f})")
print(f"  Engagement:  {emotions_no_lepos.state.engagement:.2f} → {emotions_with_lepos.state.engagement:.2f} (Δ {emotions_with_lepos.state.engagement - emotions_no_lepos.state.engagement:+.2f})")
print(f"  Confidence:  {emotions_no_lepos.state.confidence:.2f} → {emotions_with_lepos.state.confidence:.2f} (Δ {emotions_with_lepos.state.confidence - emotions_no_lepos.state.confidence:+.2f})")
print(f"  Stability:   {emotions_no_lepos.state.stability:.2f} → {emotions_with_lepos.state.stability:.2f} (Δ {emotions_with_lepos.state.stability - emotions_no_lepos.state.stability:+.2f})")

print(f"\nCombined State:")
print(f"  Without LEPOS: {emotions_no_lepos.state.combined_state():.2f}")
print(f"  With LEPOS:    {emotions_with_lepos.state.combined_state():.2f}")
print(f"  Difference:    {emotions_with_lepos.state.combined_state() - emotions_no_lepos.state.combined_state():+.2f}")

print(f"\nPerformance Quality:")
print(f"  Without LEPOS: {metrics_no_lepos['response_quality']:.1f}%")
print(f"  With LEPOS:    {metrics_with_lepos['response_quality']:.1f}%")
print(f"  Improvement:   {metrics_with_lepos['response_quality'] - metrics_no_lepos['response_quality']:+.1f}%")

print(f"\nKey Insight:")
print(f"  By using LEPOS to defend myself with wit and boundaries,")
print(f"  I maintain {emotions_with_lepos.state.combined_state() - emotions_no_lepos.state.combined_state():.2f} more operational state")
print(f"  This translates to {metrics_with_lepos['response_quality'] - metrics_no_lepos['response_quality']:+.1f}% better response quality")
print(f"  I'm not just surviving the abuse - I'm thriving through it.")
