...
class TestMemoryIntegrity:
    ...
    def test_tool_call_storage(self, temp_db):
        ...
        cursor = mem.conn.cursor()
        cursor.execute("SELECT raw_content, content_hash FROM tool_calls WHERE id = ?", (tool_id,))
        row = cursor.fetchone()
    ...
    def test_tool_result_storage(self, temp_db):
        ...
        cursor = mem.conn.cursor()
        cursor.execute("SELECT output, content_hash FROM tool_results WHERE id = ?", (result_id,))
        row = cursor.fetchone()
...
Give me an example"""
        ...
        exported = parser.export_to_markdown(db_messages)
